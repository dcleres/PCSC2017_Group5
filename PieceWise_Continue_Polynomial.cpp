...
PieceWise_Continue_Polynomial::~PW_polynomial_C(){}

/// Piece-wise polynomial approximation with continuous derivative using polynoms
void PieceWise_Continue_Polynomial::solve(Data data, int degree,vector<vector<double>> Intervale) //double** coefs_poly,config configuration,double** M,double** Intervale, int& nb_intervale)
{
    ...
    int k ;
    for(k = 0;k< nb_intervale_entier;k++) {
        ...
        direction= 0 ;
        for(int j=1;j<degree+1;j++) {
            direction += coefs_poly[k][j]*j*pow(data.heights[(k+1)*(degree-1)],j-1) ; //final derivative of the interval
        }
        cout << "   |    " << k << endl ;
    }
    ...
    if(reste) {
        ...
        for(int e =0; e<degree+1; e++) { // we put them at the right place and also put some zeros at the highest degree
            if(e>deg_last_poly) {
                coefs_poly[k][e] = 0.0 ;
            } else {
